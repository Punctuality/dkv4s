...
case class FollowerNode(node: Node,
                        currentTerm: Long,
  ...
  override def onTimer(logState: LogState,
    ...
    val (state, actions) =
      CandidateNode(node, currentTerm, logState.lastLogTerm.getOrElse(0L)).onTimer(logState, config)
      ...
      if (msg.term > currentTerm) {
        ...
        val actions =
          if (currentLeader.isEmpty)
            List(StoreState, AnnounceLeader(msg.leaderId, false))
        ...
        val (nextState, actions) =
          if (currentLeader.isEmpty)
            (
              this.copy(currentLeader = Some(msg.leaderId)),
              List(AnnounceLeader(msg.leaderId, false))
            )
          else if (currentLeader.contains(msg.leaderId))
            (this, List.empty)
          else
