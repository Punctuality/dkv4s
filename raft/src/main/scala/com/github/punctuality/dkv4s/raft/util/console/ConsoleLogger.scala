package com.github.punctuality.dkv4s.raft.util.console
    ...
    procLevel(opLevel)(
      Clock[F].realTime.flatMap(time =>
        (opLevel match {
          ...
          s"[${LocalDateTime.ofEpochSecond(time.toSeconds, 0, ZoneOffset.UTC).toLocalTime}] ($opLevel) $name - $msg" + err
