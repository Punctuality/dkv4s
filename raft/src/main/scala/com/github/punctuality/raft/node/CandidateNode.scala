...
case class CandidateNode(node: Node,
                         currentTerm: Long,
                         lastTerm: Long,
  ...
  override def onTimer(logState: LogState,
    ...
    val actions =
      config.members.toList.filterNot(_ == node).map(nodeId => RequestForVote(nodeId, request))
    val quorumSize = (config.members.size + 1) / 2

    if (1 >= quorumSize) {
      val matchIndex = config.members.filterNot(_ == node).map(n => (n, 0L)).toMap
      val nextIndex =
        config.members.filterNot(_ == node).map(n => (n, logState.lastLogIndex + 1)).toMap
      val actions = config.members
        .filterNot(_ == node)
        .map(n => ReplicateLog(n, currentTerm, logState.lastLogIndex + 1))
        .toList

      (
        LeaderNode(node, currentTerm, matchIndex, nextIndex),
        StoreState :: AnnounceLeader(node, false) :: actions
      )
    } else {
      (
        this.copy(
  ...
  override def onReceive(logState: LogState,
                         config: ClusterConfiguration,
                         msg: VoteRequest
  ...
  override def onReceive(logState: LogState,
    ...
    if (msg.term < currentTerm) {
      ...
      val actions   = List(StoreState, AnnounceLeader(msg.leaderId, false))
      ...
      if (msg.prevLogIndex > 0 && localPrvLogEntry.isEmpty)
        ...
        (
          nextState,
          (
            AppendEntriesResponse(node, msg.term, msg.prevLogIndex + msg.entries.length, true),
            actions
          )
        )
    } else {
