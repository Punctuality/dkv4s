...
import com.github.punctuality.raft.service.{ClusterConfigStorage, Log}
import com.github.punctuality.raft.model.{ClusterConfigurationCommand, Command, LogCompactionPolicy, LogEntry, Node, ReadCommand, Snapshot, WriteCommand}
import com.github.punctuality.raft.protocol.{AppendEntries, LogState}
import com.github.punctuality.raft.storage.{LogStorage, SnapshotStorage, StateMachine}
...
class LogImpl[F[_]: MonadCancel[*[_], Throwable]: Logger](
  ...
  def state: F[LogState] =
    for {
      ...
      lastTerm <-
        if (lastIndex > 0) logStorage.get(lastIndex).map(entry => Some(entry.term))
  ...
  def getAppendEntries(leaderId: Node, term: Long, nextIndex: Long): F[AppendEntries] =
    for {
      ...
      lastIndex <- logStorage.lastIndex
      lastEntry <-
        if (nextIndex > 1) logStorage.get(nextIndex - 1).map(Option(_)) else Monad[F].pure(None)
      commitIndex <- getCommitIndex
  ...
  def append[T](term: Long, command: Command[T], deferred: Deferred[F, T]): F[LogEntry] =
    transactional {
      for {
        ...
        _ <-
          Logger[F].trace(
            s"Appending a command to the log. Term: $term, Index: ${logEntry.index} -- "
          )
